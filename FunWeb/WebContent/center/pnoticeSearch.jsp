...
int count=pbdao.getBoardCount(search);
//한페이지에 보여줄 글 개수설정
int pageSize=15;
//현 페이지 번호 가져오기
		...
		<div class="col-md-8">
			...
			pBoardBean pbb = (pBoardBean)boardList.get(check);
				...
				if (pbb.getFile() != null) {  // 이미지 파일 있으면
					...
					fileName = "monster.png";// 없으면 기본 그림으로 출력
				}
				if(check % 4 == 0){
		        	%>
				...
				<div style = "text-align:center;">	
					<img src="../upload/<%=pbb.getFile() %>" width ="100" height ="100" class="img-thumbnail"></div>
				<div style = "text-align:center;"><a href = "f	content.jsp?num=<%=pbb.getNum() %>"><%=pbb.getSubject() %></a></div>
...
for(int i=startPage; i<=endPage; i++){
	%><a href="pnoticeSearch.jsp?pageNum=<%=i  %>&search=<%=search%>"><%=i %></a><%
			}
//이전
if(startPage > pageBlock){
	%><a href="pnoticeSearch.jsp?pageNum=<%=startPage-pageBlock%>&search=<%=search%>">Prev</a><%
}
//다음
if(endPage < pageCount){
	%><a href="pnoticeSearch.jsp?pageNum<%=startPage+pageBlock%>&search=<%=search%>">Next</a><%
}
	%>
